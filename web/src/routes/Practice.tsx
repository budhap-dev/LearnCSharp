...
/**
 * One place to practise: a marked exam per module, and every lesson quiz. Quizzes live on
 * the lesson pages (so the theory is a scroll away); this tab is the launchpad.
 */
export function Practice() {
  ...
  return (
    <>
      <h1>Practice</h1>
      <p className="lede">
        Two ways to test yourself: a <strong>quiz</strong> teaches, with instant explanations; an{' '}
        <strong>exam</strong> measures, with marks and a grade.
      </p>

      <section>
        <h2>Module exams</h2>
        <p className="muted">Three levels each, drawn from the whole module's question bank.</p>
        <div className="exam-launch-grid">
          {modules.map((m) => {
            const attempts = examHistory(m);
            const best = attempts.length
              ? attempts.reduce((a, b) => (b.marks / b.totalMarks > a.marks / a.totalMarks ? b : a))
              : null;
            return (
              <Link key={m} to={`/module/${m}/exam`} className="exam-launch">
                ...
                <span className="el-body">
                  ...
                  {best ? (
                    <span className="el-best">
                      ...
                      <span className={`badge grade-${best.grade.toLowerCase().replace(/\s+/g, '-')}`}>
                        {best.grade}
                      </span>
                    </span>
                  ) : (
